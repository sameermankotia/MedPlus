...
<div class="card mt-3">
    <div class="card-header"><b>Diagnosis :</b></div>
    <form method="post" action="{{ route('quiz_diag.post',$casestudies->id) }}">
        @csrf
        <div class="card-body">

            @php
                if($attempt->diagnosis != null){
                    $explode = explode(", ",$attempt->diagnosis); 
                }
            @endphp

            <div class="row">
                <div class="col-sm-8">
                    <select class="form-select select2" multiple="multiple" id="select2" name="diag[]">
